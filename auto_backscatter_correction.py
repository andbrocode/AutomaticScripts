...
def __merge_backscatter_data(tbeg, tend, ring, path_to_data):
    ...
    for dat in date_range(t1, t2):
        ...
        if process:
            ...
            if not out.empty:
                print(f" -> write to: {_path}FJ{ring}_{dat_str}_backscatter.pkl")
                out.to_pickle(f"{_path}FJ{ring}_{dat_str}_backscatter.pkl")

                # move file to tmp files
                try:
                    shutil.move(_path+filename, _path+f"tmp/"+filename)
                except:
                    print(f" -> failed to move file {filename} to {_path}tmp/")
            else:
                continue
