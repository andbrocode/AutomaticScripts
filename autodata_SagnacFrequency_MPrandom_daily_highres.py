...
def main(times):
    ...
    ## amount of samples
    _N = int(config['time_interval']/config['t_steps'])
...
#                                        )
        ...
        if loading_data_status:
            try:
                ...
                tt_utc, tt_sec, tt_mjd, ff, hh, pp, ac, dc, con = zeros(_N), zeros(_N), zeros(_N), zeros(_N), zeros(_N), zeros(_N), zeros(_N), zeros(_N), zeros(_N)
        ...
        if seed.split(".")[3] == config['seeds'][0].split(".")[3]:
            ...
            if not loading_data_status:
                f1, p1, ac_1, dc_1, con_1 =  zeros(_N), zeros(_N), zeros(_N), zeros(_N), zeros(_N)
            else:
                monos += st[0]
                f1, p1, ac_1, dc_1, con_1 = ff, pp, ac, dc, con
        elif seed.split(".")[3] == config['seeds'][2].split(".")[3]:
            if not loading_data_status:
                f2, p2, ac_2, dc_2, con_2 = zeros(_N), zeros(_N), zeros(_N), zeros(_N), zeros(_N)
            else:
                monos += st[0]
