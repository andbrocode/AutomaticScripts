...
def __merge_backscatter_data(tbeg, tend, ring, path_to_data):
    ...
    for dat in date_range(t1, t2):
        ...
        file = f"FJ{ring}_{dat_str}_backscatter_T{config['interval']}_{config['mode']}{config['alabel']}{config['project']}.pkl"
        ...
        if process:
            ...
            for m in range(24):
                ...
                filename = f"FJ{ring}_{dat_str}_{hour}_backscatter_T{config['interval']}_{config['mode']}{config['alabel']}{config['project']}.pkl"
                try:
            ...
            if not out.empty:
                print(f" -> write to: {_path}backscatter/FJ{ring}_{dat_str}_backscatter_T{config['interval']}_{config['mode']}{config['alabel']}{config['project']}.pkl")
                out.to_pickle(f"{_path}backscatter/FJ{ring}_{dat_str}_backscatter_T{config['interval']}_{config['mode']}{config['alabel']}{config['project']}.pkl")
...
def processing(arg):
    ...
    out_df.to_pickle(config['path_to_data']+f"FJ{config['ring']}_{date_str}_{time_str}_backscatter_T{config['interval']}_{config['mode']}{config['alabel']}{config['project']}.pkl")

    if config['verbose']:
        print(f" -> writing: {config['path_to_data']}FJ{config['ring']}_{date_str}_{time_str}_backscatter_T{config['interval']}_{config['mode']}{config['alabel']}{config['project']}.pkl")
...
def main(config):
    ...
    __save_to_pickle(config, config['path_to_data'], f"config{config['project']}")
